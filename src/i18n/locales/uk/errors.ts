export default {
  dashboardLoad: 'Не вдалося завантажити статистику',
  selectApp: 'Оберіть застосунок',
  invalidDuration:
    'Вкажіть загальний час від 1 хвилини до {{max}} для цього періоду',
  saveLimit: 'Не вдалося зберегти ліміт',
  desktopApiUnavailable:
    'Desktop API недоступний. Перезапустіть Limit або перевірте preload script.',
  ipcSender: 'Недозволений IPC sender',
  invalidTracking: 'Некоректне значення трекінгу',
  invalidAppId: 'Некоректний ідентифікатор застосунку',
  invalidActivity: 'Вкажіть коректну тривалість для вибраного дня.',
  activityConflict:
    'Активність змінилася під час редагування. Оновіть запис і спробуйте ще раз.',
  activityNotFound: 'Цього запису активності вже немає. Оновіть список.',
  invalidPermission: 'Некоректний тип дозволу',
  invalidAppData: 'Некоректні дані застосунку',
  invalidLimit: 'Вкажіть коректний час для обраного періоду ліміту',
  storageRead:
    'Не вдалося прочитати локальну базу. Створено резервну копію для відновлення.',
  storageImport:
    'Не вдалося імпортувати стару JSON-історію. Створено резервну копію для відновлення.',
  storageSave: 'Не вдалося зберегти локальну історію.',
  storageMaintenance: 'Не вдалося завершити збереження історії.',
  unknown: 'Сталася невідома помилка',
  dashboard: {
    loading: 'Завантажуємо ваш день...',
    title: 'Не вдалося відкрити статистику',
    unsupported: 'Трекінг не підтримується в цьому середовищі',
    unavailable: 'Трекінг тимчасово недоступний',
    retryLabel: 'Повторити перевірку',
  },
} as const;
