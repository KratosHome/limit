import type { AppLimit, AppUsage, DashboardData, DateRange, KnownApp, LimitApi, LimitInput, Settings } from '../types';
...
function makeTimeline(range: DateRange) {
  if (range.from === range.to) {
    return Array.from({ length: 24 }, (_, hour) => ({
      key: String(hour),
      seconds: hour < 8 || hour > 21 ? 0 : Math.max(0, 500 + Math.sin(hour * 1.7) * 420 + (hour % 3) * 230),
    }));
  }
  const points = [];
  ...
  while (cursor <= end && points.length < 31) {
    points.push({ key: toDayKey(cursor), seconds: 13_000 + Math.max(0, Math.sin(cursor.getDate()) * 4_500) });
...
function dashboard(range: DateRange): DashboardData {
  const multiplier = range.from === range.to ? 1 : Math.min(9, makeTimeline(range).length * 0.83);
  ...
  const knownApps: KnownApp[] = apps.map(({ id, name, category, lastSeenAt }) => ({ id, name, category, lastSeenAt }));
  return {
    apps,
    totalSeconds: apps.reduce((sum, app) => sum + app.seconds, 0),
    previousTotalSeconds: Math.round(apps.reduce((sum, app) => sum + app.seconds, 0) * 1.12),
...
export const demoApi: LimitApi = {
  async getDashboard(range) { return dashboard(range); },
  async getStatus() { return dashboard({ from: toDayKey(new Date()), to: toDayKey(new Date()) }).tracker; },
  async setTrackingEnabled(enabled) { mockSettings = { ...mockSettings, trackingEnabled: enabled }; return { ...mockSettings }; },
  async updateSettings(patch) { mockSettings = { ...mockSettings, ...patch }; return { ...mockSettings }; },
  async saveLimit(input: LimitInput) {
    const previous = mockLimits.find((limit) => limit.appId === input.appId);
    const saved: AppLimit = { ...previous, ...input, lastWarningDate: previous?.lastWarningDate ?? null, lastReachedDate: previous?.lastReachedDate ?? null, pausedDate: null };
    mockLimits = [...mockLimits.filter((limit) => limit.appId !== saved.appId), saved];
    return saved;
  },
  async deleteLimit(appId) { mockLimits = mockLimits.filter((limit) => limit.appId !== appId); return true; },
