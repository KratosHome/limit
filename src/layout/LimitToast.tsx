...
export function LimitToast({ toast, onOpenLimits }: LimitToastProps) {
  return (
    <div role="status" aria-live="assertive" className="fixed bottom-5 right-5 z-[60]">
      <Button variant="ghost" size="none" onClick={onOpenLimits} className="flex w-[340px] items-start gap-3 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 text-left shadow-[0_18px_50px_rgba(15,23,42,.18)] hover:bg-[var(--surface)]">
        ...
        <span>
          <strong className="block text-[12px] text-[var(--text)]">{toast.title}</strong>
          <span className="mt-1 block text-[10px] leading-4 text-[var(--muted)]">{toast.message}</span>
        </span>
      </Button>
    </div>
  );
