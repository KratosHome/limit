...
export function LimitModal({ apps, existing, initialAppId, onClose, onSave, onDelete }: LimitModalProps) {
  ...
  const [warningMinutes, setWarningMinutes] = useState(existing?.warningMinutes ?? 10);
  ...
  const selectedApp = useMemo(() => apps.find((app) => app.id === appId), [appId, apps]);

  useEffect(() => {
    const previousFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    const frame = window.requestAnimationFrame(() => {
      const firstControl = dialogRef.current?.querySelector<HTMLElement>('#limit-app:not(:disabled)')
  ...
  async function submit(event: React.FormEvent) {
    event.preventDefault();
    if (!selectedApp) { setError('Оберіть застосунок'); return; }
    if (minutes < 1 || minutes > 1440) { setError('Вкажіть час від 1 хвилини до 24 годин'); return; }
    setSaving(true);
    setError('');
    try {
      await onSave({ appId: selectedApp.id, appName: selectedApp.name, dailyLimitMinutes: minutes, warningMinutes: Math.min(warningMinutes, Math.max(0, minutes - 1)), enabled });
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Не вдалося зберегти ліміт');
      setSaving(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 p-5 backdrop-blur-[2px]" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <form ref={dialogRef} onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="limit-modal-title" aria-busy={saving} className="modal-panel w-full max-w-[510px] overflow-hidden rounded-[24px] border border-[var(--border)] bg-[var(--surface)] shadow-[0_28px_80px_rgba(15,23,42,.22)]">
        ...
        <div className="space-y-5 px-6 py-5">
          <div>
            <label htmlFor="limit-app" className="field-label">Застосунок</label>
            <div className="relative mt-2">
              {selectedApp && <div className="pointer-events-none absolute left-3 top-1/2 z-10 -translate-y-1/2"><AppIcon id={selectedApp.id} name={selectedApp.name} size="sm" /></div>}
              <select id="limit-app" disabled={Boolean(existing)} value={appId} onChange={(event) => setAppId(event.target.value)} className="field-input h-14 w-full pl-14 disabled:cursor-not-allowed disabled:opacity-70">
                {!apps.length && <option value="">Спочатку відкрийте потрібний застосунок</option>}
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="rounded-2xl border border-[var(--border)] bg-[var(--surface-muted)] p-4">
              <span className="mb-2 flex items-center gap-2 text-[11px] font-bold text-[var(--text)]"><BellRing size={15} className="text-[var(--accent-strong)]" /> Попередити</span>
              <select value={warningMinutes} onChange={(event) => setWarningMinutes(Number(event.target.value))} className="w-full bg-transparent text-[11px] font-semibold text-[var(--muted-strong)] outline-none"><option value={0}>Без попередження</option><option value={5}>За 5 хвилин</option><option value={10}>За 10 хвилин</option><option value={15}>За 15 хвилин</option></select>
            </label>
            <label className="flex cursor-pointer items-center justify-between rounded-2xl border border-[var(--border)] bg-[var(--surface-muted)] p-4">
              <span><span className="block text-[11px] font-bold text-[var(--text)]">Ліміт активний</span><span className="mt-1 block text-[9px] font-medium text-[var(--muted)]">Сповіщати щодня</span></span>
          ...
          <div className="flex items-start gap-2.5 rounded-xl bg-indigo-50/70 px-3.5 py-3 text-[10px] leading-4 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-200"><Info size={14} className="mt-0.5 shrink-0" /><span>Limit покаже системне сповіщення, але не закриватиме застосунок автоматично.</span></div>
          {error && <p role="alert" className="text-[10px] font-semibold text-rose-500">{error}</p>}
        </div>
