...
test('appBundlePath returns the outer macOS application bundle', () => {
  assert.equal(
    appBundlePath('/Applications/Steam.app/Contents/MacOS/Steam Helper.app/Contents/MacOS/helper'),
...
test('resolveApplicationIconPath finds a historical macOS app by name', async () => {
  const result = await resolveApplicationIconPath({
    ...
    roots: ['/Applications'],
    existsSync: (candidate) => candidate === '/Applications/Google Chrome.app',
    execute: async (executable) => ({
      stdout: executable === '/usr/libexec/PlistBuddy' ? 'com.google.Chrome\n' : '',
    }),
  });
...
test('applicationPathFromBundleId falls back to Spotlight metadata', async () => {
  const execute = async (executable, arguments_) => {
    ...
    assert.deepEqual(arguments_, ['kMDItemCFBundleIdentifier == "com.example.renamed"']);
...
test('resolveApplicationIconPath rejects a same-name app with another bundle id', async () => {
  const result = await resolveApplicationIconPath({
    ...
    execute: async (executable) => ({
      stdout: executable === '/usr/libexec/PlistBuddy' ? 'com.example.Impostor\n' : '',
    }),
  });
...
test('existingIconPath rejects Windows network and device paths', () => {
  ...
  assert.equal(existingIconPath('\\\\server\\share\\app.exe', 'win32', existsSync), null);
  assert.equal(existingIconPath('\\\\?\\C:\\app.exe', 'win32', existsSync), null);
  assert.equal(existingIconPath('C:\\Program Files\\Example\\app.exe', 'win32', existsSync), 'C:\\Program Files\\Example\\app.exe');
});
